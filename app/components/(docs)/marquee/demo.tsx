...
export default function MarqueeDemo() {
  return (
    <div className="flex h-full flex-col justify-center gap-6 p-4 sm:gap-8 sm:p-6">
      ...
      <Marquee duration={38} gap={12} reverse>
        {COMPONENTS.map((name) => (
          <span
            key={name}
            className="shrink-0 whitespace-nowrap rounded-full border border-border/70 bg-card px-4 py-1.5 text-sm font-medium text-foreground/80"
          >
            {name}
          </span>
        ))}
      ...
      <Marquee duration={20} gap={20} fade={false} pauseOnHover={false}>
        {Array.from({ length: 6 }, (_, index) => (
          <span
            key={index}
            className="shrink-0 whitespace-nowrap font-mono text-xs uppercase tracking-[0.24em] text-muted-foreground"
          >
            no fade · no pause
          </span>
        ))}
